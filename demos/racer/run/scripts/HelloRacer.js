var camera, scene, renderer, loader, stats, ambientLight, directionalLight, car, body, lfw, rfw, lrw, rrw, bm, dt, ta, debug, DEG2RAD = Math.PI / 180, DEG90 = Math.PI / 2, sa = 0, s1 = 0, fs = false, rs = false, ur = false, ul = false, uu = false, ud = false, cvel = new THREE.Vector3, vel = new THREE.Vector3, a2d = new THREE.Vector3, force = new THREE.Vector3, res = new THREE.Vector3, acc = new THREE.Vector3, ft = new THREE.Vector3, flf = new THREE.Vector3, flr = new THREE.Vector3, av = 0, aa = 0, distance = 10, height = 5, heightDamping = 2, rotationDamping = 3, randomPositionMin = new THREE.Vector3(3, 0.1, 3), randomPositionMax = new THREE.Vector3(5, 3, 5), cameraSpeedMin = 0.1, cameraSpeedMax = 0.1, cameraShotTime = 5, target = new THREE.Vector3, cameraSpeed = new THREE.Vector3(4, 2, 4), cameraTime = 0, userWeight = 1, angleX, angleY, orbitX, orbitY, orbitDistance, orbitOrigin = new THREE.Vector3(4, 2, 4), enableSkidMarks = false, maxMarks = 1024, markWidth = 0.275, numMarks = 0, lastMark = -1, markSection = {
	...
	intensity : 0,
	lastIndex : 0
}, skidmarks = [], updated = false, skm, skg, v3 = new THREE.Vector3;
if(Detector.webgl)
	init();
else {
...
function init() {
	...
	a.appendChild(stats.domElement);
	ta = (new Date).getTime();
	animate();
	pin = pad(Math.floor(Math.random() * 9999), 4);

	document.getElementById('info2').innerHTML = "To drive the racer with your smartphone, go to http://goo.gl/1a54qh and enter the PIN code " + pin;
	setTimeout(function() {
		initWS(pin)
	}, 500);
}
...
function LoadCar() {
	...
	var b = {
		...
		material : new THREE.MeshLambertMaterial({
			color : 16777215,
			map : ImageUtils.loadTexture("obj/textures/BodyPaint.jpg"),
			envMap : a,
		...
		material : new THREE.MeshPhongMaterial({
			color : 16777215,
			map : ImageUtils.loadTexture("obj/textures/Rim.jpg"),
			ambient : 4210752
		})
	}, r = {
		x : 0,
	...
	loader.loadBinary({
		model : "obj/js/Suspension.js",
		callback : function(d) {
			AddBodyPart(d, c)
		}
	});
	loader.loadBinary({
		model : "obj/js/InsideBlack.js",
		callback : function(d) {
			AddBodyPart(d, e)
		}
	});
	loader.loadBinary({
		model : "obj/js/GlossyBlack.js",
		callback : function(d) {
			AddBodyPart(d, f)
		}
	});
	loader.loadBinary({
		model : "obj/js/Chrome.js",
		callback : function(d) {
			AddBodyPart(d, o)
		}
	});
	loader.loadBinary({
		model : "obj/js/Bolts.js",
		callback : function(d) {
			AddBodyPart(d, p)
		}
	});
	loader.loadBinary({
		model : "obj/js/Windshield.js",
		callback : function(d) {
			AddBodyPart(d, i)
		}
	});
	loader.loadBinary({
		model : "obj/js/RearLight.js",
		callback : function(d) {
			AddBodyPart(d, g)
		}
	});
	loader.loadBinary({
		model : "obj/js/RearLightGlass.js",
		callback : function(d) {
			AddBodyPart(d, k)
		}
	});
	loader.loadBinary({
		model : "obj/js/SteeringWheel.js",
		callback : function(d) {
			AddBodyPart(d, l)
		}
	});
	loader.loadBinary({
		model : "obj/js/DriverBody.js",
		callback : function(d) {
			AddBodyPart(d, m)
		}
	});
	loader.loadBinary({
		model : "obj/js/Helmet.js",
		callback : function(d) {
			AddBodyPart(d, h)
		}
	});
	loader.loadBinary({
		model : "obj/js/Visor.js",
		callback : function(d) {
			AddBodyPart(d, j)
		}
	});
	loader.loadBinary({
		model : "obj/js/Tyre.js",
		callback : function(d) {
			AddWheelPart(d, n)
		}
	});
	...
	loader.loadBinary({
		model : "obj/js/WheelBase.js",
		callback : function(d) {
			AddWheelPart(d, r)
		}
	});
	loader.loadBinary({
		model : "obj/js/HelloEnjoy.js",
		callback : function(d) {
			AddHelloEnjoy(d)
		}
	});
...
function SteerWheels() {
	var a = 2.5 * dt, b = ul ? -1 : 0, c = ur ? 1 : 0;
	b = b + c;
	if(b == 0)
		a = Math.min(a, Math.abs(s1));
	if(b > s1)
		s1 += a;
	else if(b < s1)
		s1 -= a;
...
function UpdateCar() {
	var a = 5, b = 30, c = -5, e = -5.2, f = 2, o = 1, p = 1, i = o + p, g = car.rotation.y, k = Math.sin(g), l = Math.cos(g);
	...
	if(force.x == 0 && vel.x == 0)
		flf.y = flr.y = force.y = a = acc.x = acc.y = aa = a2d.x = a2d.y = cvel.x = cvel.y = av = 0;
	else {
		a = o * flf.y - p * flr.y;
		acc.x = force.x / h;
		acc.y = force.y / h;
	...
	if(enableSkidMarks && Math.abs(i) > 0.5)
...
function UpdateCamera() {
	...
	if(cameraTime < 0) {
		...
		var a = camera.position.clone(), b = car.position.clone();
		if(b.length > 100) {
			a -= b;
			b = new THREE.Vector3
		}
		for(var c = a.clone(), e = 200; --e > 0 && a.distanceTo(c) < 3; ) {
			...
			c.y = b.y + randomPositionMin.y + Math.random() * (randomPositionMax.y - randomPositionMin.y);
...
function onKeyDown(a) {
	var b = a.keyCode;
	if(b == 38 || b == 87)/* Up arrow was pressed. */
		uu = true;
	else if(b == 40 || b == 83)/* Down arrow was pressed */
		ud = true;
	else if(b == 37 || b == 65)/* Left arrow was pressed. */
		ur = true;
	else if(b == 39 || b == 68)/* Right Arrow was pressed . */
		ul = true;
	else if(b == 32)
		rs = true;
...
function onKeyUp(a) {
	a = a.keyCode;
	if((a == 38 || a == 87) && upState == 0)
		uu = false;
	else if((a == 40 || a == 83) && downState == 0)
		ud = false;
	else if((a == 37 || a == 65) && leftState == 0)
		ur = false;
	else if((a == 39 || a == 68) && rightState == 0)
		ul = false;
	else if(a == 32)
		rs = false
}
	...
	if(updated) {
		...
		for(var a = 0, b = skg.vertices, c = 0; c < numMarks && c < maxMarks; c++) {
			if(skidmarks[c].lastIndex != -1 && skidmarks[c].lastIndex > numMarks - maxMarks) {
				var e = skidmarks[c], f = skidmarks[e.lastIndex % maxMarks];
				...
				b[a * 4 + 3] = new THREE.Vertex(new THREE.Vector3(e.posr.x, e.posr.y, e.posr.z));
...
function initWS(pin) {
	...
	function handleMessage(message) {
		...
		var temp = message.getText().split('&');
		moveLog("Message received: State=" + message.getText());

		if(temp[0] == 1) {
			ur = true;
		}/* Left tilt was detected. */
		else {
			ur = false;
		}
...
function doConsumeMessage( message )
{
	...
	// Decode from ArrayBuffer to String
	// Turn String into JSON data
