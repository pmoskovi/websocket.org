...
var BLOCK_SIZE = 4;
var BOARD_HEIGHT = 20;  
var CONTROLLER_PATH = 'http://kaazing.org/demos/tetris/run/controller.html';
var FREQUENCY_DOWN = 0.10;
var FREQUENCY_SIDE = 0.15;

// Kaazing Gateway constants
...
function do_connect()
{
  ...
  kaazing.subscribe( TETRIS_TOPIC + '_' + user );
...
function do_window_load()
{
  ...
  // Populate controller link (visible URL)
  link = document.querySelector( '.url > a' );
  link.href = CONTROLLER_PATH + '?user=' + user;
  link.innerHTML = CONTROLLER_PATH + '?user=' + user;
  
  // Populate QRCode element with actual QRCode
  // Client side library used
  // No server needed
  qrcode = new QRCode( 'code', {
    text: CONTROLLER_PATH + '?user=' + user,
    width: 300,
    height: 300,
